<?php
get_header();
get_template_part('elements/common/page-visual'); ?>
	<section class="common-section single-section">
		<div class="common-section__inner">
			<div class="post-content">
				<?php the_content(); ?>
			</div>

			<?php
			$prevPost = get_previous_post();
			$prevPostUrl = $prevPost ? $prevPost->guid : '';
			$nextPost = get_next_post();
			$nextPostUrl = $nextPost ? $nextPost->guid : '';
			?>
			<div class="pager">
				<div class="pager-left<?php if (!$prevPost) {
					echo ' is-disabled';
				} ?>">
					<a href="<?php echo $prevPostUrl; ?>">
						<span>PREV</span>
					</a>
				</div>
				<div class="pager-center">
					<a href="<?php echo home_url('/news/'); ?>"><span>お知らせ一覧へ戻る</span></a>
				</div>
				<div class="pager-right<?php if (!$nextPost) {
					echo ' is-disabled';
				} ?>">
					<a href="<?php echo $nextPostUrl; ?>">
						<span>NEXT</span>
					</a>
				</div>
			</div>

		</div>
	</section>
<?php get_footer(); ?>